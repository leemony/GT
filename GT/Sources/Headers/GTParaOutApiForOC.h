...
/**
 * @brief   csv文件的删除移动(支持OC语法)。 // 20170104.leemon
 * @ingroup GT输出参数使用说明
 *
 * @param fromDir [NSString *] csv文件源目录名
 * @param toDir [NSString *] csv文件目标目录名
 * @return
 *
 * Example Usage:
 * @code
 *    //清理autoSave目录，使用tempSave暂存上次获取的历史数据
 *    GT_OC_AUTOSAVE_SET(@"autoSave", @"tempSave");
 * @endcode
 */
#define GT_OC_AUTOSAVE_SET(fromDir,toDir) func_moveCSVToTempDir(fromDir, toDir)
FOUNDATION_EXPORT void func_moveCSVToTempDir(NSString *fromDir, NSString *toDir);
