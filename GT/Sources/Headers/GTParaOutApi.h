...
/**
 * @brief   设置自动保存CSV文件（CPU，MEM，NET等）间隔     20170104.leemon
 * @ingroup GT输出参数使用说明
 *
 * @param interval [double] 间隔，单位为s，最小值：10(10s) 最大值:600(10min)
 * @return
 *
 * Example Usage:
 * @code
 *    //设置自动保存CSV文件间隔为30s
 *    GT_OUT_SAVE_CSV_INTERVAL_SET(30);
 * @endcode
 */
#define GT_OUT_SAVE_CSV_INTERVAL_SET(interval) func_setAutoSaveCSVInterval(interval)
extern void func_setAutoSaveCSVInterval(double interval);
