...
@synthesize autoSaveCsvInterval = _autoSaveCsvInterval;
...
- (id)init
{
    ...
    if (self) {
        ...
        // 自动保存csv文件相关配置
        _autoSaveCsvInterval = 0;   // 默认为0，关闭自动保存
        _lastAutoSave = 0;
...
- (void)dirDelIfExists:(NSString *)dirPath
{// 20170104.leemon
    NSFileManager * manager = [NSFileManager defaultManager];
    BOOL result = YES;
    if([manager fileExistsAtPath:dirPath isDirectory:&result]) {
...
void func_setAutoSaveCSVInterval(double interval)
{// 20170104.leemon
    if ((interval < 10) || (interval > 600)) {
        return;
    }
    [[GTConfig sharedInstance] setAutoSaveCsvInterval:interval];
}
