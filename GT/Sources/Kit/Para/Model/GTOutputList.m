...
#pragma mark 维护 autoSave 目录相关文件
//- (void)filemoveThreadStart:(BOOL)inThread
//{
//    if (inThread) {
//        NSThread *thread = [[[NSThread alloc] initWithTarget:self selector:@selector(fileMoveThreadProc:) object:nil] autorelease];
//        thread.name = @"fileMoveThreadProc";
//        [thread start];
//    } else {
//        [self fileMoveThreadProc];
//    }
//}

- (void)fileMoveThreadProc:(NSString *)fromDir toDir:(NSString *)toDir
{
    @autoreleasepool {
        
        NSString *autoSaveDir = [NSString stringWithFormat:@"%@%@/%@", [[GTConfig sharedInstance] usrDir], M_GT_PARA_OUT_DIR, fromDir];
        NSString *tempSaveDir = [NSString stringWithFormat:@"%@%@/%@", [[GTConfig sharedInstance] usrDir], M_GT_PARA_OUT_DIR, toDir];
        
        // 对应目录不存在则创建一个新的autoSaveDir目录
        NSFileManager * manager = [NSFileManager defaultManager];
        BOOL isDir = YES;
        if (![manager fileExistsAtPath:autoSaveDir isDirectory:&isDir]) {
            [manager createDirectoryAtPath:autoSaveDir withIntermediateDirectories:YES attributes:nil error:nil];
        }
        else {
            // autoSaveDir目录不为空
            NSArray * array = [manager contentsOfDirectoryAtPath:autoSaveDir error:nil];
            if ([array isKindOfClass:[NSArray class]] && array.count > 0) {

                // 清理tempSaveDir
                if([manager fileExistsAtPath:tempSaveDir isDirectory:&isDir]) {
                    [manager removeItemAtPath:tempSaveDir error:nil];
                }
                [manager createDirectoryAtPath:tempSaveDir withIntermediateDirectories:YES attributes:nil error:nil];
                
                // 将autoSaveDir下的文件移动至tempSaveDir
                for (NSString * filename in array) {
                    NSString * fileFromPath = [autoSaveDir stringByAppendingPathComponent:filename];
                    NSString * fileToPath = [tempSaveDir stringByAppendingPathComponent:filename];
                    if ([manager fileExistsAtPath:fileFromPath]){
                        if ([[filename pathExtension] isEqualToString:M_GT_FILE_TYPE_CSV]) {
                            // 移动csv
                            [manager moveItemAtPath:fileFromPath toPath:fileToPath error:nil];
                        }
                    }
                }
            }
...
#pragma mark - OC
// 20170104.leemon
void func_moveCSVToTempDir(NSString *fromDir, NSString *toDir)
{
    @autoreleasepool {
//        [[GTOutputList sharedInstance] filemoveThreadStart:NO];
        [[GTOutputList sharedInstance] fileMoveThreadProc:fromDir toDir:toDir];
    }
}
