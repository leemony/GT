...
#import "GTConfig.h"
#import "GTOutputList.h" // for filemoveThreadStart.20170104.leemon
...
//@synthesize autoSaveOn = _autoSaveOn;
//@synthesize autoSaveInterval = _autoSaveInterval;
//@synthesize lastAutoSave = _lastAutoSave;
...
- (BOOL)needAutoSave
{
    NSTimeInterval interval = [[GTConfig sharedInstance] autoSaveCsvInterval];
    if (interval != 0) {
        if ([[GTConfig sharedInstance] lastAutoSave] == 0) {
            [[GTConfig sharedInstance] setLastAutoSave:[GTUtility timeIntervalSince1970]];
            return YES;
        }
        if (([GTUtility timeIntervalSince1970] - [[GTConfig sharedInstance] lastAutoSave]) > interval) {
            [[GTConfig sharedInstance] setLastAutoSave:[GTUtility timeIntervalSince1970]];
            return YES;
        }
    }
...
- (void)threadProc:(id)obj
{
    while (TRUE) {
        ...
        @autoreleasepool {
            ...
            // 保存csv文件.20170103.leemon
            if ([self needAutoSave] == YES) {
                GT_OC_AUTOSAVE_SET(@"autoSave", @"tempSave");
                GT_OC_OUT_HISTORY_SAVE_ALL(@"autoSave");
            }
        }
    }
}
